...
- `make build` to build all three distribution flavors without network access;
...
`make all` produces three standalone executable flavors and three adjacent
SHA-256 files beneath `dist/`: a fully documented
`letsencrypt-routeros.dev.bash`, the conventional comment-stripped
`letsencrypt-routeros.bash`, and `letsencrypt-routeros.min.bash` generated by
the pinned Bash-Minifier dependency.  Build provenance is carried in executable
variables so it survives comment removal and minification.  `dist/` is generated
derivative state and is not committed.

The historical root `letsencrypt-routeros.bash` path remains the public
sourceable/executable entry point and the file used by the container image.  It
corresponds to the ordinary flavor with stable development provenance.

### Release Artifacts

Release publication uses a validate-then-publish workflow.  Candidate artifacts
are built and tested without release-write permission, transferred as exact
workflow-artifact bytes, re-verified, and then attached to the GitHub release.
The privileged publication job does not rebuild them.

Each release includes all three executable flavors and their adjacent
`.sha256` files.
